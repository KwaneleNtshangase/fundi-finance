...
import { useFundiState as useFundiStateInternal } from "@/hooks/useFundiState";
import type { FundiState } from "@/hooks/useFundiState";
import type { Route } from "@/app/pageViews.types";

export const FundiContext = createContext<FundiState | null>(null);

export function FundiProvider({ children }: { children: React.ReactNode }) {
  const state = useFundiStateInternal();
  ...
  return (
    <FundiContext.Provider value={value}>
      {children}
    </FundiContext.Provider>
  );
}
...
export function useFundi() {
  ...
  if (!context) {
    throw new Error("useFundi must be used within a FundiProvider");
  }
  return context;
}
